...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ error: "Bad request", message: "rating must be 1-5" }, { status: 400 })
  }

  const result = await submitRating(bookingId, body.rating, body.comment, body.feedbackTags)
